-- ============================================================
-- CAMPUSCONNECT — FACULTY COVERAGE SECURITY HARDENING
--
-- Goals:
--   1. Coverage state changes happen through controlled RPCs.
--   2. Uncovered requests can be retried.
--   3. Previously declined faculty are not silently re-offered.
--   4. Candidates must teach in the same department.
--   5. Permanent timetable remains unchanged.
--
-- HOD authorization is intentionally not added here because
-- CampusConnect does not yet have a verified auth.uid() -> HOD
-- identity mapping.
-- ============================================================


-- ============================================================
-- 1. REMOVE BROAD DIRECT UPDATE POLICIES
-- ============================================================
...
-- ============================================================
-- 2. CONTROLLED COVERAGE REQUEST CANCELLATION
-- ============================================================

create or replace function
public.cancel_faculty_coverage_request(
...
begin
  ...
  then
    raise exception
      'You do not have permission to cancel this coverage request';
  end if;
  ...
  if request_row.status = 'Covered' then
    raise exception
      'A covered request cannot be cancelled through this action';
  end if;
...
revoke all
on function
public.cancel_faculty_coverage_request(uuid)
from public;
...
-- Existing response RPC remains the only supported Faculty
-- path for accepting or declining an offer.
