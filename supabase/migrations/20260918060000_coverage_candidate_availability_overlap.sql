...
begin
  ...
  with eligible as (
    ...
    where
      ...
      -- Faculty availability is matched by actual time overlap,
      -- not by exact period-slot UUID. This is necessary because
      -- an imported/published timetable entry can differ slightly
      -- from the configured scheduling slot while still referring
      -- to the same teaching period.
      and exists (
        select 1
        from
          public.timetable_period_slots available_slot
        where
          available_slot.id =
            availability.period_slot_id

          and available_slot.profile_id =
            scheduling_profile_id

          and available_slot.is_teaching_slot =
            true
