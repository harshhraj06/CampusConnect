-- ============================================================
-- CAMPUSCONNECT
-- Fix ambiguous identifiers in faculty assignment RPC.
--
-- Root cause:
-- RETURNS TABLE output names such as batch_subject_id and
-- faculty_id conflicted with columns referenced inside PL/pgSQL.
-- ============================================================

drop function if exists
public.assign_faculty_teaching_allocation(
  uuid,
  uuid,
  text,
  text,
...
create function public.assign_faculty_teaching_allocation(
...
begin
  ...
  select abs.*
  into v_subject
  from public.attendance_batch_subjects as abs
  where abs.id =
    p_batch_subject_id;
  ...
  if not found then
    raise exception
      'Selected account is not a Faculty account.';
  end if;
